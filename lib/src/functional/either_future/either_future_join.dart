...
/// Joins the left side of one [EitherFuture] (L) with the left side of another
/// [EitherFuture] (S). The other Either is only invoked if the first one is left.
class _JoinAsyncLeftEitherFuture<S, L, R> implements EitherFuture<Tuple<L, S>, R> {
  final FutureOr<EitherFuture<L, R>> source;
  final LazyFuture<EitherFuture<S, R>> _joiningLazy;

  _JoinAsyncLeftEitherFuture({
    required this.source,
...
/// Joins the right side of one [EitherFuture] (R) with the right side of another
/// [EitherFuture] (S). The other Either is only invoked if the first one is right.
class _JoinAsyncRightEitherFuture<S, L, R> implements EitherFuture<L, Tuple<R, S>> {
  final FutureOr<EitherFuture<L, R>> source;
  final LazyFuture<EitherFuture<L, S>> _joiningLazy;

  _JoinAsyncRightEitherFuture({
    required this.source,
...
extension EitherFutureJoiningExtension<L, R> on EitherFuture<L, R> {
  ...
  /// consumed again.
  EitherFuture<Tuple<L, S>, R> joinLeft<S>(FutureOr<Either<S, R>> other()) =>
  ...
  /// the result is then stored and used again when the joined [Either] is
  /// consumed again.
  EitherFuture<L, Tuple<R, S>> joinRight<S>(FutureOr<Either<L, S>> other()) =>
  ...
  EitherFuture<Tuple<L, S>, R> joinLeftAsync<S>(FutureOr<EitherFuture<S, R>> other()) =>
      _JoinAsyncLeftEitherFuture(source: this, joining: other);
  ...
  EitherFuture<L, Tuple<R, S>> joinRightAsync<S>(FutureOr<EitherFuture<L, S>> other()) =>
      _JoinAsyncRightEitherFuture(source: this, joining: other);
}

extension FutureOfEitherFutureJoiningExtension<L, R> on FutureOr<EitherFuture<L, R>> {
  /// Joins the left side of one [EitherFuture] (L) with the left side of another
  /// [Either] (S). The other Either is only invoked if the first one is left.
  ///
  /// The [other] function is only invoked once to create or get the [Either]
